...
- **Both tile sheets grew from 576 to 608 pixels** to fit the new art &mdash; 1,216 tiles now, of which 17 are still free. Worth knowing before adding more: every extra row costs startup time, since `TileSet` slices each tile through `canvas.toDataURL` at roughly 5.5ms a tile across two sheets.
